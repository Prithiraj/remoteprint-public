...
from app_ver_2.utilities.convert import Convert
...
api = Namespace('cloudlabelprint', description='webhook, cloud print related operations')

@api.route('/')
class CloudLabelPrintAPI(Resource):
    ...
    def get(self):
        try:
            ...
            # api.logger.info("OUTPUT CONTENT TYPE " + content_type)
            
            image_file = open(ticketDesign.markup, 'rb')
            print("CLOUD LABEL PRINT")
            print(ticketDesign.markup)
            data = image_file.read()
            response = Response(data)
            print(data)
            image_file.close()
            response.headers["Content-Type"] = 'image/jpeg' 
            response.headers["X-Star-Cut"] = "none" 
            # if queue == 4:
