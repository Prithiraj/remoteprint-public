...
from app_ver_1.dbmanager.database import Base
from app_ver_1.dbmanager.database import db_session as db
from app_ver_1.dbmanager.database import dbtype, session_manager
...
class Device(Base):
    ...
    def setDeviceStatus(cls, devicemac, status):
        try:
            ...
            data = cls(devicemac=devicemac).update(status=status, last_poll=datetime.utcnow())
            return data
        except ValueError:
            raise ValueError("invalid value provided")
        except Exception as e:
